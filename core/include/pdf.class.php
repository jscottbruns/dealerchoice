...
require_once(SITE_ROOT.'core/include/pdfXtra.class.php');
/**
* Cpdf
*
...
function ezTable(&$data,$cols='',$title='',$options=''){
  ...
  $defaults = array(
    'shaded'=>0,	// Ticket #64 - Remove shading from alternate lines
	'showLines'=>1,'shadeCol'=>array(0.8,0.8,0.8),'shadeCol2'=>array(0.7,0.7,0.7),'fontSize'=>10,'titleFontSize'=>12,'titleJustification'=>'left'
  ...
  if (strlen($title)){
	...
	if ($options['titleJustification'] && $options['titleJustification'] == 'center')
		$putX = $middle-$w/2;
	else
		$putX = $baseX0;

    $this->addText($putX,$this->y,$options['titleFontSize'],$title);
...
function ezImage($image,$pad = 5,$width = 0,$resize = 'full',$just = 'center',$border = ''){
	...
	if (stristr($image,'://'))//copy to temp file
	{
		...
		while(!feof($fp))
   		...
   		fclose($fp);
		$image = tempnam (UPLOAD_DIR, "php-pdf");
		$fp2 = @fopen($image,"w");
   		fwrite($fp2,$cont);
  		fclose($fp2);
		$temp = true;
