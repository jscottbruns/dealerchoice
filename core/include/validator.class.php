...
//	Returns true if string contains only numbers
	...
	function is_allnumbers ($text)
	{
		...
		$this->ERROR = "Value must contain only numbers";
		return false;
	}
...
//	Returns true if string contains only letters
	...
	function is_allletters ($text)
	{
		...
		$this->ERROR = "Value must contain only letters";
		return false;
	}
...
//For purposes of validating percentages for discounts, invalid is greater than 100 or less than 1.
	function is_percent($num = "") {
		if(empty($num))
		{
			return false;
		}
...
//	Looks for [something]@[valid hostname with DNS record]
	...
	function is_email ($Address = "")
	{
		...
		if(!ereg("@",$Address))
		{
			$this->ERROR = "Invalid, no @ symbol in string";
			return false;
		}
...
//	urlencoded data.
	...
	function is_url ($Url = "")
	{
		...
		{
			$this->ERROR = "Parse error reading [$Url]";
			return false;
		}
		...
		if(empty($scheme))
		{
			$this->ERROR = "Missing protocol declaration";
			return false;
		}

		if(empty($HostName))
		{
			$this->ERROR = "No hostname in URL";
			return false;
		}

		if (!eregi("^(ht|f)tp",$scheme))
		{
			$this->ERROR = "No http:// or ftp://";
			return false;
		}
...
//	the internet, or if there are connection problems. (firewall etc)
	...
	function url_responds ($Url = "")
	{
		...
		if(!$fd)
		{
			$this->ERROR = "Failed : $php_errormsg";
			return false;
		}
		else
		{
...
//	validity as a reachable internet host
	...
	function is_hostname ($hostname = "")
	{
		...
		if(empty($hostname))
		{
			$this->ERROR = "No hostname submitted";
			return false;
		}
		...
		if(empty($tld))
		{
			$this->ERROR = "No TLD found in [$hostname]";
			return false;
		}
		...
		if(!$this->is_bigfour($tld))
		{
			...
			{
				$this->ERROR = "Unrecognized TLD [$tld]";
				return false;
			}
		}
...
//	************************************************************
	...
	function is_bigfour ($tld)
	{
		...
		$BigFour = array (com=>com,edu=>edu,net=>net,org=>org,mil=>mil,us=>us,ca=>ca,uk=>uk,gov=>gov,biz=>biz,info=>info,me=>me);
		$tld = strtolower($tld);
		...
		// Commenting out to allow all extentions
		//return false;
		return true;
	}
...
//	Does not check for reserved or unroutable IPs.
	...
	function is_ipaddress ($IP = "")
	{
		...
		if(empty($IP))
		{
			$this->ERROR = "No IP address submitted";
			return false;
		}
		//	123456789012345
		//	xxx.xxx.xxx.xxx
		...
		if(!empty($Bad))
		{
			$this->ERROR = "Bad data in IP address [$Bad]";
			return false;
		}
		$chunks = explode(".",$IP);
		$count = count($chunks);
		...
		if ($count != 4)
		{
			$this->ERROR = "Not a dotted quad [$IP]";
			return false;
		}
		...
		while ( list ($key,$val) = each ($chunks) )
		{
			if(ereg("^0",$val))
			{
				$this->ERROR = "Invalid IP segment [$val]";
				return false;
			}
			$Num = $val;
			settype($Num,"integer");
			if($Num > 255)
			{
				$this->ERROR = "Segment out of range [$Num]";
				return false;
			}
...
//	IP address is valid, and resolves to a hostname? true or false
	...
	function ip_resolves ($IP = "")
	{
		...
		if($Hostname == $IP)
		{
			$this->ERROR = "IP does not resolve.";
			return false;
		}
		...
		if($Hostname)
		{
			...
			//{
				//$this->ERROR = "no DNS records for resolved hostname [$Hostname]";
				//return false;
			//}
			//if( (gethostbyname($Hostname)) != $IP )
			//{
				//$this->ERROR = "forward:reverse mismatch, possible forgery";
				//	Non-fatal, but it should be noted.
			//}
		}
		else
		{
			$this->ERROR = "IP address does not resolve";
			return false;
		}
...
//	************************************************************
	...
	function browser_gen ()
	{
		...
		if(empty($client))
		{
			$this->ERROR = "No User Agent for Client";
			return $generation;
		}
...
//	United States valid state code? true or false
	...
	function is_state ($State = "")
	{
		...
		if(empty($State))
		{
			$this->ERROR = "No state submitted";
			return false;
		}
		if( (strlen($State)) != 2)
		{
			$this->ERROR = "Too many digits in state code";
			return false;
		}
		...
		{
			$this->ERROR = "Unrecognized state code [$State]";
			return false;
		}
...
//	Valid postal zip code? true or false
	...
	function is_zip ($zipcode = "")
	{
		...
		if(empty($zipcode))
		{
			$this->ERROR = "Zip code: No zipcode submitted";
			$f = false;
		}
		...
		if(!empty($Bad))
		{
			$this->ERROR = "Invalid data in zipcode field [$Bad]";
			$f = false;
		}
		...
		if ( ($len > 10) or ($len < 5) )
		{
			$this->ERROR = "Invalid length [$len] for zipcode code";
			$f = false;
		}

		$canada = preg_match("/^[a-z]\d[a-z] ?\d[a-z]\d$/i",$zipcode);
...
//	this listing in here. :)
	...
	function is_country ($countrycode = "")
	{
		...
		{
			$this->ERROR = "No country code submitted";
			return $Return;
		}
		...
		{
			$this->ERROR = "2 digit codes only [$countrycode]";
			return $Return;
		}
		...
		{
			$this->ERROR = "Unrecognized country code [$countrycode]";
			$Return = "";
		}
