...
class dC_AJAX_XML_Builder extends dC_XML_Builder
{
    function dC_AJAX_XML_Builder($content_type = '', $charset = '') {
        if ($content_type)
        {
    ...
    function array2xml($array) {
        ...
        if ( defined('DEBUG') && $_POST['aa_sfunc_args'][0] != 'poll' ) {
            global $db, $pun_start;

            list($usec, $sec) = explode(' ', microtime());
            $time_diff = sprintf('%.3f', ( (float)$usec + (float)$sec ) - $pun_start);
            $array['html']['debug_info_holder'] = "<p>[ Generated in $time_diff seconds, " . $db->get_num_queries() . " queries executed, Rand ID " . rand(50, 5000) . " ]</p>\n";

            if ( defined('PUN_SHOW_QUERIES') )
                $array['html']['debug_info_holder'] .= "<p>" . display_saved_queries() . "</p>";
...
class dC_XML_Parser
{
    ...
    */
    function dC_XML_Parser($xml, $path = '')
    {
        if ($xml !== false)
        {
            $this->xmldata = $xml;
...
class XMLexporter extends dC_XML_Builder
{
}
?>